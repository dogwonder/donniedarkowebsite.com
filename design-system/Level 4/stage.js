...
(function () {
  function fitStage() {
    ...
    if (!stage) return;
    var still = /[?&]still\b/.test(location.search);   /* spread thumbnail */
    var pad = still ? 0 : 24;   /* fill the thumbnail edge-to-edge; letterbox when played */
    var sx = (window.innerWidth - pad * 2) / 800;
    var sy = (window.innerHeight - pad * 2) / 500;
